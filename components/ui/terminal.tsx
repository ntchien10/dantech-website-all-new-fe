...
export const AnimatedSpan = ({
  ...
  return (
    <motion.div
      ...
      className={cn("grid font-normal tracking-tight text-xs sm:text-[13px] leading-relaxed break-words whitespace-normal", className)}
      onAnimationComplete={() => {
        if (!sequence) return
...
export const Terminal = ({
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "z-0 h-full w-full rounded-2xl border",
        className
      )}
    >
      <div className="flex flex-row items-center justify-between border-b border-white/[0.08] px-4 py-3 bg-black/30">
        ...
        <div className="text-[11px] font-mono text-slate-400 font-medium select-none">
          dantech-engine ~ zsh
        </div>
        <div className="w-12"></div>
      </div>
      <pre className="px-5 py-4 overflow-hidden">
        <code className="grid gap-y-1.5 sm:gap-y-2 font-mono text-xs sm:text-[13px] leading-relaxed overflow-x-hidden">
          {wrappedChildren}
        </code>
      </pre>
    </div>
